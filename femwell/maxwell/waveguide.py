...
class Mode:
    ...
    def calculate_effective_area(self, field="xy", equation=2):
        """
        :param equation: the equation used to calculate the effective area
        """
        if equation == 1:
            @Functional(dtype=complex)
            def top(w):
                return cross(w['E'][0], w['H'][0])

            @Functional(dtype=complex)
            def bottom(w):
                return cross(w['E'][0], w['H'][0]) ** 2

            return top.assemble(self.basis, E=self.E, H=self.H) ** 2 / bottom.assemble(self.basis, E=self.E, H=self.H)
